...
import { ActivityIndicator,Alert, StyleSheet, Platform, SafeAreaView, FlatList, Modal, StatusBar, View, ImageBackground, Text, Image, Dimensions, TouchableOpacity, Linking, KeyboardAvoidingView, } from 'react-native';
...
export default class Chat extends React.Component {
    ...
    componentDidMount() {
        AsyncStorage.getItem("user_id")
            .then(user_id => {
                ...
                console.log("state this.props.route.params.room_id============" + this.props.route.params.room_id);
                AsyncStorage.getItem("token")
                    .then(token => {
                        //var tokens = JSON.parse(token)
                        this.setState({ token: token });
                        console.log("state token============" + token);
                        console.log("state roomDetails===== %%%%=======" + JSON.stringify(this.props.route.params.roomDetails));
    ...
    reportUserApi() {
        console.log("data reportVideoApi "+JSON.stringify({
            ...
            "reason": 'other',
            "id":this.props.route.params.roomDetails.OtherId
        }))
        this.setState({
            isLoading_2: true
        })
            ...
            .then((responseData) => {
                ...
                if (responseData.status === true) {
                    this.setState({
                        ...
                        profile_block: responseData.user.blockedUsers.includes(this.props.route.params.roomDetails.OtherId)
                    })
    ...
    messages(user_id, token) {
        ...
        fetch(strings.base_Url + 'messagesDetails/' + this.props.route.params.room_id, {
            method: 'POST',
            headers: {
                'Accept': 'application/json',
                'Content-Type': 'application/json',
    ...
    submitChatMessage(type, msg) {
        var msg_sent = {
            ...
            messageType: type,
            format: 'png',
            recieverId: this.props.route.params.roomDetails.OtherId,
            receiverName: this.props.route.params.receiver_name,
            receiverProfile: this.props.route.params.receiver_profile,
            senderId: this.state.user_id,
            senderName: this.state.senderName,
            senderProfile: this.state.senderProfile,
            productID: this.props.route.params.roomDetails.productID,
            productName: this.props.route.params.roomDetails.productName,
        }
        ...
        launchImageLibrary(options, response => {
            ...
            if (response.didCancel) {
               ...
               this.cpmpress(response.assets[0].uri)
                ...
                // format = response.assets[0].type
                //  this.submitChatMessage('video', response.assets[0].base64)
        ...
        launchImageLibrary(options, response => {
            ...
            if (response.didCancel) {
               ...
               // console.log('User tapped custom button: ', response.customButton
                ...
                console.log("image selelect" + response.assets[0].fileSize)
                ImageResizer.createResizedImage(response.assets[0].uri,  200,200,"JPEG", 100,0,)
                .then(response => {
                  console.log("ImageResizer 1 $$$$ "+ JSON.stringify(response))
                  RNFetchBlob.fs
    ...
    async cpmpress(videoUrl){
        ...
        console.log("result this.props.route.params.videoUrl  metaData :$$%%%%%:::" + videoUrl)
    ...
    blockUser() {
        ...
        fetch(strings.base_Url + 'blockUser', {
            ...
            body: JSON.stringify({
                ...
                "blockId": this.props.route.params.receiver_id
            })
        })
            .then((response) => response.json())
            .then((responseData) => {
                ...
                <View style={{ flex: 1, flexDirection: 'column', backgroundColor: '#fff' }}>
                    ...
                    <View style={{ width: '90%', alignSelf: 'center', backgroundColor: '#fff' }}>
                        {this.state.list.length != 0 && (
                            <View style={{ marginTop: 20, marginBottom: 10 }}>
                                <Text style={[styles.font_regu, { fontSize: 20, color: '#000', marginLeft: 20, fontWeight: 'bold', textTransform: 'capitalize' }]}>{this.props.route.params.receiver_name}</Text>
                                            ...
                                            <View style={{ flexDirection: 'row', padding: 8, borderRadius: 10, }}>
                                                <View style={{ minheight: 30, padding: 5, maxWidth: width * 80 / 100, }}>
                                                    ...
                                                    {item.messageType == 'image' && (
                                                        <TouchableOpacity onPress={() => this.props.navigation.navigate("ChatVideoPlay", { video_url: item.message })} style={{ width: 150, height: 150, backgroundColor: 'rgba(180,180,180,0.4)', borderRadius: 4, padding: 4 }}>
                                                            <Image source={{ uri: strings.base_video + item.message }} style={{ width: '100%', height: '100%', }} />
                                                        </TouchableOpacity>
                                                    )
                                                    }
                                                    {item.messageType == 'video' && (
                                                        <TouchableOpacity onPress={() => this.props.navigation.navigate("ChatVideoPlay", { video_url: item.message })} style={{ alignItems: 'center', justifyContent: 'center', width: 150, height: 150, backgroundColor: 'rgba(180,180,180,0.4)', borderRadius: 4, padding: 4 }}>
                    ...
                    {this.state.profile_block === false ?
                        <KeyboardAvoidingView behavior='padding' keyboardVerticalOffset={50}>
                            <View style={{ backgroundColor: '#fff', justifyContent: 'flex-end', bottom: 0 }}>
                                    ...
                                    <View style={{ width: '95%', height: 40, borderRadius: 50, marginLeft: 5, marginRight: 5, marginTop: 5, flexDirection: 'row' }}>
                                        <View style={{ position: 'absolute', alignItems: 'center', flexDirection: 'row', justifyContent: 'space-between', bottom: 0, width: width * 95 / 100, alignSelf: 'center', borderRadius: 20, height: 45 }}>
                                            <TextInput
                                                ...
                                                style={[ {
                                                    color: '#000', padding: 10, paddingLeft: 20, width: width * 78 / 100 - 30, fontSize: 16, backgroundColor: 'rgba(180,180,180,0.1)', elevation: 0, borderRadius: 50,
                    ...
                    <View style={{ width: '100%', justifyContent: 'center', flexDirection: 'column', borderWidth: 0.5, borderColor: 'white', borderRadius: 4, }}>
                        <View style={{ backgroundColor: '#fff', marginBottom: 0, height: 100, alignItems: 'center' }}>
                            <View style={{ alignSelf: 'center', justifyContent: 'space-around', width: '95%', height: 140, borderRadius: 50, marginLeft: 5, marginRight: 5, marginTop: 5, flexDirection: 'row' }}>
                                <TouchableOpacity onPress={() => this.chooseFromGallary3()} style={{ width: 100, alignItems: 'center' }}>
                                    <Image source={require('../assets/picture.png')} style={{ width: 50, height: 50, resizeMode: 'contain', }} />
                                    <Text style={[styles.font_regu, { color: '#000', paddingLeft: 10, paddingRight: 20, paddingTop: 10, paddingBottom: 10, fontSize: 16, }]}>
                                        {"l'image"}
                                    </Text>
                                </TouchableOpacity>
                                <TouchableOpacity onPress={() => this.chooseFromGallary()} style={{ width: 100, alignItems: 'center' }}>
                ...
                {this.state.profileblockModel === true && (
                    <View style={{ alignItems: 'center', justifyContent: 'flex-end', position: 'absolute', width: '100%', height: '100%', backgroundColor: 'rgba(0,0,0,0.4)' }}>
                        <View style={[{ width: '80%', alignItems: 'center', justifyContent: 'center', borderRadius: 20, backgroundColor: '#fff' }]}>
                            <TouchableOpacity onPress={() => this.props.navigation.navigate("SeeUserProfile", { other_user_id: this.props.route.params.receiver_id })} style={{ marginTop: 30 }} >
