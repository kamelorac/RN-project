    ...
    chooseFromGallary() {
        ...
        launchImageLibrary(options, response => {
            ...
            if (response.didCancel) {
                ...
                var image_url = data_url =  response.assets[0].uri.replace('file://', ''); 
                    this.uploadImage(RNFetchBlob.wrap(image_url))
                // ,
            }
        });
    }
            ...
            <SafeAreaView style={{ flex: 1, alignItems: 'center', backgroundColor: colors.testColorWhite }}>
                <View style={{ width: '90%', height: 60, alignItems: 'center', flexDirection: 'row', justifyContent: 'space-between' }}>
                    ...
                    <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                        <TouchableOpacity onPress={() => this.props.navigation.navigate("Setting")} style={{ marginRight: 8, width: 35, height: 35, borderRadius: 50 / 2, backgroundColor: 'rgba(0,0,0,0.8)', justifyContent: 'center', alignItems: 'center', }}>
                            <Image style={{ width: 20, height: 20, resizeMode: 'contain', tintColor: 'white' }} source={require('../assets/g.png')} />
                        </TouchableOpacity>
                        <TouchableOpacity onPress={() => this.props.navigation.navigate("EnRegister")} style={{ width: 35, height: 35, borderRadius: 50 / 2, backgroundColor: 'rgba(0,0,0,0.8)', justifyContent: 'center', alignItems: 'center', }}>
                ...
                <ScrollView>
                    <View style={{ width: '100%' }}>
                        ...
                        <View style={{ alignSelf: 'center', marginTop: 30, alignItems: 'center', width: '90%', flexDirection: 'row', justifyContent: 'space-between' }}>
                            <TouchableOpacity onPress={() => this.props.navigation.navigate("Subscriptions")} style={{ width: '48%', height: 55, backgroundColor: 'rgba(0,0,0,0.8)', justifyContent: 'center', alignItems: 'center', borderRadius: 50, }}>
                                <Text style={[styles.font_regu, { fontSize: 16, color: '#fff' }]}>{this.state.listofSubscribedUsers_count} Abonnements</Text>
                            </TouchableOpacity>
                            <TouchableOpacity onPress={() => this.props.navigation.navigate("Subscribers")} style={{ width: '48%', height: 55, backgroundColor: colors.themeColor, justifyContent: 'center', alignItems: 'center', borderRadius: 50, }}>
                                        ...
                                        renderItem={({ item, index }) =>
                                            <View style={{ width: '50%', alignItems: 'center' }}>
                                                <View style={{ width: width * 40 / 100, alignItems: 'center', marginTop: 20 }}>
                                                    <TouchableOpacity
                                                        //onPress={() => this.props.navigation.navigate("WebVideoPlay", { videoUrl: item.videoslink[0], posterImage:  strings.base_video+item.thumbnail, product_id: item._id, save:'true'})} 
                                                        onPress={() => this.props.navigation.navigate("ProductEdit", { videoUrl: item.videoslink[0], posterImage: strings.base_video + item.thumbnail, product_id: item._id })}
