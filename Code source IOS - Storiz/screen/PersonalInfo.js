...
import {
    StyleSheet, StatusBar, ActivityIndicator, TouchableOpacity, SafeAreaView, View, Text, Image, Modal, FlatList, KeyboardAvoidingView
} from 'react-native';
        ...
        fetch(strings.base_Url + 'show-user/' + user_id, {
            ...
            .then((responseData) => {
                ...
                if (responseData.status === true) {
                    var str = responseData.user.address;
                    var temp = new Array();
                    temp = str.split(",");
    ...
    updateUserProfile() {
        ...
        fetch(strings.base_Url + 'update-user-profile', {
            ...
            body: JSON.stringify({
                ...
                "contactNumber": this.state.phone_number,
                "address": this.state.location +","+ this.state.rue,
                "country": this.state.count_name,
                "city": this.state.ville,
                "rue": this.state.rue,
                "postal_code": this.state.code_postal,
    ...
    render() {
        ...
        return (
            <SafeAreaView style={{ flex: 1, alignItems: 'center', backgroundColor: colors.testColorWhite }}>
                ...
                <TouchableOpacity onPress={() => this.props.navigation.goBack()} style={{ alignSelf: 'flex-start', marginLeft: 15, width: 40, height: 40, alignItems: 'center', justifyContent: 'center', marginTop: 10 }}>
